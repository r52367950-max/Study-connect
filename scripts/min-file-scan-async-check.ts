...
class PrismaMock {
  ...
  fileScanJob = {
    ...
    updateMany: async ({ where, data }: any) => {
      // Mirror claimPendingJobs' conditional claim: succeed only while the job is
      // still claimable (PENDING/RUNNING), ignoring nextRunAt timing so the test
      // can drive successive retry ticks synchronously.
      if (
        this.job &&
        this.job.id === where.id &&
        (this.job.status === FileScanJobStatus.PENDING || this.job.status === FileScanJobStatus.RUNNING)
      ) {
