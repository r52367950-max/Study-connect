...
class PrismaMock {
  ...
  fileScanJob = {
    ...
    // Lenient claim mock — matches by id only. The real WHERE guards (status /
    // nextRunAt / stale-lock) are exercised by min-file-scan-claim-concurrency.
...
async function assertEicarFileFails() {
  ...
  // Terminal verdicts land in DEAD_LETTER (kept for manual inspection).
